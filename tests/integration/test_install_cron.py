...
def test_install_cron_script_renders_expected_template(tmp_path: Path) -> None:
    ...
    uv_bin.chmod(uv_bin.stat().st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
    ...
    assert str(uv_bin) in rendered
    assert "run --no-sync python" in rendered
...
""".lstrip(),
    ...
    assert "run --no-sync python" in installed_cron
