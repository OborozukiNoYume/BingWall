...
install-cron:
	$(UV_RUN) scripts/install_cron.py --install --app-dir "$(CRON_APP_DIR)" --venv-python "$(CRON_VENV_PYTHON)" --log-dir "$(CRON_LOG_DIR)" --env-file "$(CRON_ENV_FILE)" --crontab-bin "$(CRONTAB_BIN)"
