...
- 尚未完成 `npm test` 与浏览器冒烟依赖的可复现自动化安装链路
...
1. 先明确最小告警方案，并完成至少一次真实测试通知
2. 再沉淀部署、恢复演练、`cron` 首轮验证与回滚的固定记录模板
3. 最后正式化 `npm test` 与 Playwright 依赖安装链路
