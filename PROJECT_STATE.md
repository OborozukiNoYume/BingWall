...
- 当前阶段：阶段三功能已完成，真实目标机 `139.224.235.228:8000` 已完成 `H5` 长驻部署与公网接入，并已根据 `2026-04-04` 目标机执行报告回写 `H4` 首轮 `cron` 闭环记录；`M4` 所需的最小告警方案已在运维文档中收敛为“Webhook + 外层巡检/监控”，并已通过 Server 酱完成 1 次真实测试通知；`M5` 所需的关键运维动作模板已补充到 [docs/operations-record-templates.md](/home/ops/Projects/BingWall/docs/operations-record-templates.md)；`L2` 所需的搜索与查询性能基线已补充到 [docs/benchmark-report.md](/home/ops/Projects/BingWall/docs/benchmark-report.md)；`L3` 所需的密码哈希升级路径已补充到 [docs/password-hash-migration.md](/home/ops/Projects/BingWall/docs/password-hash-migration.md)；`L5` 所需的前端源码 / 构建产物边界与提交流程已补充到 [docs/frontend-build-boundary.md](/home/ops/Projects/BingWall/docs/frontend-build-boundary.md)
- 状态说明：仓库已完成一期系统设计总纲、配套文档骨架、经系统级校准的阶段路线图，并已落地 `T1.1` 的最小后端工程骨架、`T1.2` 的 SQLite 迁移基线、`T1.3` 的 Bing 采集、去重、任务记录、图片下载与资源入库主链路、`T1.4` 的公开 API 最小集、`T1.5` 的基础公开前端、`T1.6` 的单机部署模板与自动化部署验收，以及 `T2.1` 的管理员认证与会话控制、`T2.2` 的后台内容管理 API / 页面与审计查询、`T2.3` 的手动采集任务与后台观测、`T2.4` 的健康检查、资源巡检与本地资源归档清理闭环、`T2.5` 的备份恢复与恢复演练闭环、`T3.1` 的标签体系、`T3.2` 的多来源采集、`T3.3` 的资源派生版本、`T3.4` 的 OSS / CDN 兼容资源定位、`T3.5` 的下载登记与后台统计、`T3.6` 的关键词搜索增强，以及公开 API 的地区 / 日期精确查询补齐；公开列表页现已保留固定 8 个地区选项与按日期查找入口，地区筛选继续使用 `market_code` 过滤分页列表，或按精确日期查找单张公开壁纸；当前 Bing 手动采集在未显式指定 `MARKET` 时，会按 `BINGWALL_COLLECT_BING_MARKETS` 依次抓取固定 8 个地区，并已补齐 `make browser-smoke` / `npm run browser-smoke` 的 Playwright 浏览器冒烟入口与后台登录示例模板；另外已把项目的 Python 环境准备流程正式收敛为 `uv sync` + `uv.lock`，删除了旧的 `requirements.lock.txt` 和冗余开发引导脚本，并把本地开发、`systemd` 服务模板、`cron` 模板和相关验收脚本统一切到 `uv run --no-sync python` 执行；当前已移除独立环境排障文档，相关运行说明统一收敛到主 README、部署文档和项目状态记录；`H5` 所需的真实目标机长期驻留部署与公网接入已在 `2026-04-04` 落地到阿里云 Ubuntu 服务器 `139.224.235.228`，当前公网入口为 `http://139.224.235.228:8000`；同时，`H4` 所需的首轮 `cron` 闭环验证也已根据同日目标机执行报告回写到仓库，记录文件为 `docs/h4-cron-first-run-record-2026-04-04.md`，其中保留了当前目标机采用 `ubuntu` 用户、`/home/ubuntu/BingWall` 直接部署、无额外反向代理的差异说明；同时保留 GitHub Actions 仓库级权限与 `main` 分支保护一键脚本，用于把“允许 Actions 创建 PR、禁止直接推送、必须 PR 合并、必须通过 `verify`、必须至少 1 次 Review”的规则应用到远端仓库；当前 `dev` 已吸收 `main` 最新历史并完成本地冲突整理，后续远端是否能成功建单，仍取决于 Secret 是否已配置，以及仓库是否已启用“允许 GitHub Actions 创建或批准 PR”
...
- 已提供 `/`、`/wallpapers`、`/wallpapers/{wallpaper_id}` 公开页面，以及 `/admin/login`、`/admin`、`/admin/wallpapers`、`/admin/wallpapers/{wallpaper_id}`、`/admin/change-password`、`/admin/tags`、`/admin/tasks`、`/admin/tasks/{task_id}`、`/admin/download-stats`、`/admin/logs`、`/admin/audit-logs` 后台页面；公开列表页与后台内容页均已增加关键词搜索输入，且公开首页已新增“今日壁纸 API”“随机壁纸 API”快捷入口；同时公开列表页保留固定 8 个地区选项，通过公开列表接口按 `market_code` 过滤结果；还新增日期选择器，并单独调用 `/api/public/wallpapers/by-date/{wallpaper_date}` 精确显示所选日期公开壁纸
- 已提供 `scripts/dev/playwright_smoke.js`、`scripts/dev/playwright_smoke_with_admin.example.sh` 与 `make browser-smoke` / `npm run browser-smoke` 浏览器冒烟入口，默认覆盖公开首页、公开列表筛选、壁纸详情页和后台登录页壳；当提供后台账号后，还可继续验证真实后台登录跳转
...
- `make run`、`deploy/systemd/bingwall-api.service` 与部署文档中的监听口径现已统一读取 `BINGWALL_APP_HOST` / `BINGWALL_APP_PORT`；仓库推荐口径仍为 `127.0.0.1:8000`，若目标机需要直接开放公网端口，则应显式改为 `0.0.0.0:8000` 并同步收紧安全组 / 防火墙
- `.env.example` 与 `deploy/systemd/bingwall.env.example` 已补齐 `BINGWALL_COLLECT_NASA_APOD_*`、管理员初始化和 `OSS` 相关键；真实生产值仍需在 `/etc/bingwall/bingwall.env` 中按目标机实际填写
...
- `docs/frontend-build-boundary.md`：前端源码、构建产物和提交边界说明
...
- 已将 Bing 手动采集默认市场口径从单地区收敛为固定 8 个地区；未显式传入 `MARKET` 时，会按 `BINGWALL_COLLECT_BING_MARKETS` 依次执行多地区采集，并在终端输出聚合后的任务汇总
...
- 已将公开列表页收敛为单一“地区”筛选入口，固定提供 `zh-CN / en-US / ja-JP / en-GB / de-DE / fr-FR / en-CA / en-AU` 八个选项，并继续支持与关键词、标签、分辨率和分页条件组合查询
...
- 已新增 `scripts/dev/playwright_smoke.js`、`scripts/dev/playwright_smoke_with_admin.example.sh`、`make browser-smoke` 与 `npm run browser-smoke`，把公开首页、列表筛选、详情页与后台登录页壳的浏览器检查收敛为统一入口
...
- 已实现 `/admin/login`、`/admin`、`/admin/wallpapers`、`/admin/wallpapers/{wallpaper_id}`、`/admin/change-password` 与 `/admin/audit-logs` 后台页面，并通过 `web/admin/assets/admin.js`、`web/admin/assets/pages/*.js`、`web/admin/assets/modules/core.js` 与 `web/admin/assets/admin.css` 约束页面仅调用后台 API，不直接访问数据库或图片目录
...
- 已新增 `/api/public/wallpapers/today` 与 `/api/public/wallpapers/random`，两者都返回与公开详情一致的单条壁纸结构；其中 `today` 优先返回 UTC 当天公开内容，若当天没有则回退到 UTC 昨天，并在候选日期内优先站点默认市场，`random` 仅从当前公开可见内容中随机选取
...
- 当前关键词搜索继续采用 SQLite `LIKE` 保守实现；已基于 `12,000` 条壁纸样本建立离线基线，若关键词场景超阈值或 `wallpapers` 规模接近 `50,000` 行，再评估额外索引或 `SQLite FTS`
...
- 最小告警当前采用“运维值班群 Webhook + 外层 URL 监控/主机巡检”的保守方案，不在一期仓库内引入 SMTP、告警中心或消息队列

## 后续优先级

1. 先收口生产机日志轮转方案，并把触发条件和处理步骤写入运维文档
