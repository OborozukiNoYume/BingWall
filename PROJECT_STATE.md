...
- 已新增 `.github/workflows/auto-create-pr.yml`，在 `dev` 分支 `CI` 成功且 `dev` 领先于 `main` 时自动检查并创建 `dev -> main` PR，避免重复开单
- 已新增 `scripts/github/apply_main_branch_protection.sh`，可通过 GitHub REST API 一次性把 `main` 分支设置为强制 PR、强制 `verify` 检查、强制至少 `1` 次 Review，且对管理员同样生效
