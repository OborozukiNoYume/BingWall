...
- 已提供 `/api/public/wallpapers`、`/api/public/wallpaper-filters`、`/api/public/tags`、`/api/public/site-info`、`/api/public/wallpapers/{wallpaper_id}` 和 `/api/public/download-events` 公开接口，其中列表默认返回缩略图资源，并支持 `keyword`、`tag_keys`、地区和分辨率组合查询；详情区分预览图与下载图，下载登记接口负责记录行为并返回静态资源地址
- 已提供 `/api/admin/auth/login`、`/api/admin/auth/logout`、`/api/admin/wallpapers`、`/api/admin/wallpapers/{wallpaper_id}`、`/api/admin/wallpapers/{wallpaper_id}/status`、`/api/admin/wallpapers/{wallpaper_id}/tags`、`/api/admin/tags`、`/api/admin/tags/{tag_id}`、`/api/admin/collection-tasks`、`/api/admin/collection-tasks/{task_id}`、`/api/admin/collection-tasks/{task_id}/retry`、`/api/admin/logs`、`/api/admin/audit-logs` 和 `/api/admin/download-stats` 后台接口，其中内容列表已支持 `keyword + 状态` 联合检索
