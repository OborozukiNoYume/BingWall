...
- 更新时间：2026-03-29T04:33:18Z
- 当前阶段：阶段三功能已完成，仓库内已补齐每日固定日期采集任务脚本与 `cron` 模板，运维侧仍需完成目标机安装与启用
- 状态说明：仓库已完成一期系统设计总纲、配套文档骨架、经系统级校准的阶段路线图，并已落地 `T1.1` 的最小后端工程骨架、`T1.2` 的 SQLite 迁移基线、`T1.3` 的 Bing 采集、去重、任务记录、图片下载与资源入库主链路、`T1.4` 的公开 API 最小集、`T1.5` 的基础公开前端、`T1.6` 的单机部署模板与自动化部署验收，以及 `T2.1` 的管理员认证与会话控制、`T2.2` 的后台内容管理 API / 页面与审计查询、`T2.3` 的手动采集任务与后台观测、`T2.4` 的健康检查、资源巡检与本地资源归档清理闭环、`T2.5` 的备份恢复与恢复演练闭环、`T3.1` 的标签体系、`T3.2` 的多来源采集、`T3.3` 的资源派生版本、`T3.4` 的 OSS / CDN 兼容资源定位、`T3.5` 的下载登记与后台统计、`T3.6` 的关键词搜索增强
...
- 部署形态：单机部署
- 操作系统：Ubuntu
- 后端：Python `3.14.2` + FastAPI `0.118.3` + Starlette `0.47.3` + Pillow `12.1.1`
...
- 已提供 `make setup`、`make db-migrate`、`make collect-bing`、`make collect-nasa-apod`、`make create-scheduled-collection-tasks`、`make scheduled-collect`、`make consume-collection-tasks`、`make inspect-resources`、`make archive-wallpapers`、`make backup`、`make restore`、`make verify`、`make verify-deploy`、`make verify-backup-restore`、`make run` 统一命令入口
...
- 已提供 `/`、`/wallpapers`、`/wallpapers/{wallpaper_id}` 公开页面，以及 `/admin/login`、`/admin`、`/admin/wallpapers/{wallpaper_id}`、`/admin/change-password`、`/admin/tags`、`/admin/tasks`、`/admin/tasks/{task_id}`、`/admin/download-stats`、`/admin/logs`、`/admin/audit-logs` 后台页面；公开列表页与后台内容页均已增加关键词搜索输入，且公开首页已新增“今日壁纸 API”“随机壁纸 API”快捷入口
...
- 已提供 `scripts/create_scheduled_collection_tasks.py` 与 `deploy/cron/bingwall-cron`，可按当天 UTC 日期为已启用来源自动生成 `trigger_type = cron` 的固定日期采集任务；其中 Bing 会按 `BINGWALL_COLLECT_BING_MARKETS` 为每个市场分别建任务，并用 `BINGWALL_COLLECT_BING_SCHEDULED_BACKTRACK_DAYS` 生成回溯窗口；消费该类任务时，若当天无上游图片，则会在最近 `8` 天窗口内自动回退到最近可用日期并记录日志
...
- 已新增 `app/services/scheduled_collection.py`、`scripts/create_scheduled_collection_tasks.py`、`deploy/cron/bingwall-cron` 与 `tests/integration/test_scheduled_collection.py`，让仓库能按 UTC 当天自动创建固定日期的 `scheduled_collect` 任务；其中 Bing 会按市场列表分别建任务、把回溯天数写入任务快照，并在同来源同市场同窗口已有非失败 cron 任务时保守跳过，避免重复堆积
