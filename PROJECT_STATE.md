...
- 当前阶段：阶段一 `T1.6` 已完成仓库内模板与说明，待目标机部署验收
- 状态说明：仓库已完成一期系统设计总纲、配套文档骨架、经系统级校准的阶段路线图，并已落地 `T1.1` 的最小后端工程骨架、`T1.2` 的 SQLite 迁移基线、`T1.3` 的 Bing 采集、去重、任务记录、图片下载与资源入库主链路、`T1.4` 的公开 API 最小集、`T1.5` 的基础公开前端，以及 `T1.6` 的单机部署模板与运行说明；当前剩余工作是目标机上的 Nginx 与 systemd 实机验收
...
- 阶段二开始前，仍需在真实环境完成 Nginx 装载、systemd 托管与公网访问验证
...
- 尚未在当前仓库环境内完成真实 Nginx 启动验证，因为系统中未安装 `nginx`
