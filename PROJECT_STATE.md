...
- 当前阶段：阶段一 `T1.2` 已完成，准备进入 `T1.3`
- 状态说明：仓库已完成一期系统设计总纲、配套文档骨架、经系统级校准的阶段路线图，并已落地 `T1.1` 的最小后端工程骨架与基础验证命令，以及 `T1.2` 的 SQLite 版本化迁移基线、核心表结构与数据库初始化入口
...
- 已提供 `make setup`、`make db-migrate`、`make verify`、`make run` 统一命令入口
...
- 尚未落地公开 API 和后台 API
- 尚未实现采集任务、资源管理、后台登录、就绪/深度健康检查、备份恢复
- 尚未落地生产环境部署脚本与运行清单
...
1. 先进入 `T1.3`，完成 Bing 采集、去重与资源入库主链路
2. 再补齐阶段二后台能力与运维能力
3. 最后开展阶段三增强能力
