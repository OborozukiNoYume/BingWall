...
- 已新增迁移 `V0007__image_resource_download_resolution_variants.sql` 与 Bing 多分辨率下载链路：同一张 Bing 壁纸可把多个官方分辨率分别保存为多条 `download` 资源，公开详情返回默认下载地址和 `download_variants` 列表，下载登记接口可按指定 `resource_id` 跳转对应分辨率
