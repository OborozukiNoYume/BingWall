...
- 当前阶段：阶段二进行中，`T2.1` 已完成，下一优先级为 `T2.2`
- 状态说明：仓库已完成一期系统设计总纲、配套文档骨架、经系统级校准的阶段路线图，并已落地 `T1.1` 的最小后端工程骨架、`T1.2` 的 SQLite 迁移基线、`T1.3` 的 Bing 采集、去重、任务记录、图片下载与资源入库主链路、`T1.4` 的公开 API 最小集、`T1.5` 的基础公开前端、`T1.6` 的单机部署模板与自动化部署验收，以及 `T2.1` 的管理员认证与会话控制
...
- 已提供 `/api/admin/auth/login` 与 `/api/admin/auth/logout` 后台认证接口
...
- 尚未落地后台内容管理 API
- 尚未实现后台内容管理、手动任务消费 cron、就绪/深度健康检查、备份恢复
...
1. 先进入 `T2.2`，完成后台内容管理 API 与页面
2. 再进入 `T2.3`，完成手动采集任务创建、查询与重试
3. 最后开展阶段二、阶段三其余能力补齐
