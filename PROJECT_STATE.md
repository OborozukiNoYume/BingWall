...
- 已新增 `.python-version`、`.nvmrc`、`pyproject.toml` 和 `.env.example`，并把当前文档中的 Python 环境准备口径统一切换为 `uv + .venv`
- 已实现统一配置加载入口，覆盖服务监听、数据库路径、存储目录和安全配置的必填校验
- 已实现最小 FastAPI 服务、根路由和 `/api/health/live` 健康检查
- 已补齐 `uv python install 3.14`、`uv venv --python 3.14 .venv`、`uv pip install --python .venv/bin/python -e ".[dev]"` 环境准备说明，以及 `make verify`、`make run` 命令入口和 `pytest`、`ruff`、`mypy` 基础校验
