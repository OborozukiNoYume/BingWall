...
- 已提供 `scripts/create_scheduled_collection_tasks.py` 与 `deploy/cron/bingwall-cron`，可按当天 UTC 日期为已启用来源自动生成 `trigger_type = cron` 的固定日期采集任务，并给出目标机 `cron` 安装示例；消费该类任务时，若当天无上游图片，则会在最近 `8` 天窗口内自动回退到最近可用日期并记录日志
