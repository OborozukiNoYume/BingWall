...
- 已补齐 `uv python install 3.14`、`uv sync --python 3.14 --frozen` 环境准备说明，并把 `make setup` 切换为 `uv sync --frozen` 实现；同时移除旧的 `requirements.lock.txt` 和 `scripts/dev/bootstrap.sh`，保留 `make verify`、`make run` 命令入口和 `pytest`、`ruff`、`mypy` 基础校验
