...
- 当前 `deploy/systemd/bingwall-api.service` 仍把生产 `uvicorn` 监听地址固定为 `127.0.0.1:8000`，不会直接读取 `BINGWALL_APP_HOST` / `BINGWALL_APP_PORT`；若目标机需要改监听地址或端口，必须同步修改 `systemd` 与 `nginx` 模板
- 当前 `.env.example` 已包含 `BINGWALL_COLLECT_NASA_APOD_*` 本地示例，但 `deploy/systemd/bingwall.env.example` 仍未预填这些键；若目标机需要显式关闭 NASA APOD、替换 API Key 或调整其超时 / 重试参数，需在 `/etc/bingwall/bingwall.env` 中手工补充
