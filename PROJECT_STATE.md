...
- 已提供 `make setup`、`make db-migrate`、`make collect-bing`、`make collect-nasa-apod`、`make create-scheduled-collection-tasks`、`make scheduled-collect`、`make consume-collection-tasks`、`make inspect-resources`、`make backup`、`make restore`、`make verify`、`make verify-deploy`、`make verify-backup-restore`、`make run` 统一命令入口
