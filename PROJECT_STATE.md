...
- 当前阶段：阶段二进行中，`T2.3` 已完成，下一优先级为 `T2.4`
- 状态说明：仓库已完成一期系统设计总纲、配套文档骨架、经系统级校准的阶段路线图，并已落地 `T1.1` 的最小后端工程骨架、`T1.2` 的 SQLite 迁移基线、`T1.3` 的 Bing 采集、去重、任务记录、图片下载与资源入库主链路、`T1.4` 的公开 API 最小集、`T1.5` 的基础公开前端、`T1.6` 的单机部署模板与自动化部署验收，以及 `T2.1` 的管理员认证与会话控制、`T2.2` 的后台内容管理 API / 页面与审计查询、`T2.3` 的手动采集任务与后台观测
...
- 已提供 `make setup`、`make db-migrate`、`make collect-bing`、`make consume-collection-tasks`、`make verify`、`make verify-deploy`、`make run` 统一命令入口
...
- 后续仍需补齐 `/api/health/ready`、`/api/health/deep` 与目标机 cron 安装配置
...
- 尚未实现就绪/深度健康检查、备份恢复，以及目标机 cron 安装配置
...
1. 先进入 `T2.4`，补齐就绪检查、深度检查与基础巡检
2. 再进入 `T2.5`，补齐备份恢复和恢复演练
3. 最后开展阶段三能力扩展
