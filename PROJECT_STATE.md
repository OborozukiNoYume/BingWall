...
- 已提供 `scripts/create_scheduled_collection_tasks.py` 与 `deploy/cron/bingwall-cron`，可按当天 UTC 日期为已启用来源自动生成 `trigger_type = cron` 的固定日期采集任务，并给出目标机 `cron` 安装示例
...
- 已新增 `app/services/scheduled_collection.py`、`scripts/create_scheduled_collection_tasks.py`、`deploy/cron/bingwall-cron` 与 `tests/integration/test_scheduled_collection.py`，让仓库能按 UTC 当天自动创建固定日期的 `scheduled_collect` 任务，并在同来源同日期已有非失败 cron 任务时保守跳过，避免重复堆积
...
- 已新增迁移 `V0007__image_resource_download_resolution_variants.sql` 与 Bing 多分辨率下载链路：同一张 Bing 壁纸会按官方 15 种分辨率口径分别保存为多条 `download` 资源，公开详情返回默认下载地址和 `download_variants` 列表，下载登记接口可按指定 `resource_id` 跳转对应分辨率
