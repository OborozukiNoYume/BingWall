...
| `L1` | 拆分超大模块 | `todo` | 部分完成 | 已完成后台前端脚本模块拆分；仍存在较大文件，如 `app/services/source_collection.py`、`app/repositories/collection_repository.py` |
...
- 状态：`todo`
- 完成记录：已完成 `web/admin/assets/admin.js` 拆分，当前改为“入口 + 页面模块 + 共享模块”结构；`admin.js` 已降至小入口文件
- 前置依赖：建议在高优先级完成后执行
- 阻塞后续：无
...
- `app/services/source_collection.py` 与 `app/repositories/collection_repository.py` 仍属于后续可继续收口的大文件，本条目暂不标记为完成
