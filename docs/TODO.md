...
子任务：
- [ ] 创建与 `docs/module-overview.md` 一致的后端目录骨架，并预留 `app/`、`scripts/`、`tests/`、`deploy/` 基础位置
- [ ] 固定单一 Python 精确版本并写入项目运行时管理文件，确保后续依赖安装基于同一版本
- [ ] 实现统一配置加载入口，覆盖服务监听、数据库路径、存储目录和安全配置的必填校验
- [ ] 增加最小 FastAPI 应用启动代码，并暴露可返回基础 HTTP 响应的健康占位路由
- [ ] 编写格式化命令、基础测试命令和本地启动命令入口，统一开发执行方式
- [ ] 验证缺失关键配置时启动失败、本地启动可返回最小响应、格式化与基础测试命令可成功执行
