...
子任务：
- [x] 创建公开前端首页、列表页、详情页和空状态页的页面骨架，并约束页面只通过公开 API 取数
- [x] 实现首页最新壁纸列表渲染，展示标题、日期、地区和跳转入口
- [x] 实现列表页筛选与结果刷新交互，对接公开筛选接口和列表分页参数
- [x] 实现详情页内容展示与下载入口渲染，按 API 返回控制下载按钮可见性
- [x] 增加空结果、内容不存在和资源不可用场景的明确提示页面或组件
- [x] 验证前端页面未直接访问数据库或文件系统，且首页、筛选、详情和异常提示链路可正常工作
