...
子任务：
- [ ] 实现 Bing 元数据拉取入口，并支持按地区参数获取上游壁纸数据
- [ ] 编写上游字段到内部壁纸与资源结构的映射逻辑，统一生成 `source_key`、日期、尺寸和来源信息
- [ ] 实现基于 `source_type + wallpaper_date + market_code` 和辅助哈希的双层去重判断
- [ ] 创建采集任务与逐条明细写入流程，记录成功、重复、失败和失败原因
- [ ] 实现图片下载到临时目录、完整性校验和转入正式资源目录的入库链路
- [ ] 增加下载重试、失败落盘和资源状态同步刷新逻辑，确保 `image_status` 与 `resource_status` 联动
- [ ] 验证首次采集、重复采集、下载失败和超出重试上限四类结果是否满足任务统计与状态要求
