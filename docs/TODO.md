...
子任务：
- [x] 按部署目录约定整理应用、数据、图片、日志和配置目录，并明确应用写入与 Nginx 读取权限
- [x] 编写 Nginx 路由配置，分别代理公开页面、公开 API 和正式资源目录访问
...
子任务：
- [x] 创建后台登录请求与响应 schema，明确账号密码输入、会话令牌返回和过期时间字段
- [x] 实现管理员密码摘要校验与账号状态检查，确保数据库仅保存密码摘要
- [x] 落地 `admin_sessions` 会话记录与令牌摘要存储，支持签发、过期判断和主动失效
- [x] 实现后台登录、登出和鉴权中间层，为后台接口注入当前管理员上下文
- [x] 增加登录失败、会话过期和无权限访问的统一错误码与响应处理
- [x] 验证正确登录、错误登录、会话过期拦截和登录审计记录是否符合鉴权约定
...
子任务：
- [x] 定义后台内容列表、详情、状态切换和审计查询接口的请求与响应 schema
- [x] 实现后台内容列表与详情查询服务，返回展示字段、来源字段、资源信息、当前状态和失败原因
- [x] 实现启用、禁用和逻辑删除的状态切换逻辑，并校验非法状态流转
- [x] 增加每次状态变更的审计日志写入，记录操作者、目标对象、前后状态和 `trace_id`
- [x] 实现后台内容管理页面和审计查询页面，确保页面仅通过后台 API 操作数据
- [x] 验证状态切换结果、审计查询能力和被禁用或删除内容从公开接口消失的联动行为
...
子任务：
- [x] 实现手动采集任务创建接口，校验来源、地区、日期范围和 `force_refresh` 参数后写入 `queued` 任务
- [x] 编写 cron 消费入口，按任务队列拉起既有采集主链路并更新 `running`、`succeeded`、`partially_failed`、`failed` 状态
- [x] 实现后台任务列表、任务详情和日志查询接口，返回任务统计、错误摘要和逐条处理明细
- [x] 实现按任务 ID 触发的重试接口，并记录 `retry_of_task_id` 关联关系
- [x] 创建后台任务观测页面，展示成功数、重复数、失败数、错误摘要和任务明细入口
- [x] 增加任务失败定位和日志关联逻辑，确保后台可定位失败原因
- [x] 验证任务创建、任务消费、任务重试和失败明细查询是否满足后台观测验收标准
...
子任务：
- [x] 实现 `/api/health/live`、`/api/health/ready`、`/api/health/deep` 三类健康检查接口及响应摘要结构
- [x] 在就绪检查中增加数据库连接、关键目录可访问性和必要配置可用性校验
- [x] 在深度检查中汇总最近一次采集任务状态、磁盘使用情况和资源目录摘要信息
- [x] 编写资源巡检脚本，校验数据库资源记录与正式资源目录文件的一致性
- [x] 实现巡检异常后的状态联动处理，将缺失资源对应内容下线或从公开查询中排除
- [x] 验证健康检查响应、巡检日志输出和资源丢失后的公开查询隔离行为是否符合要求
...
子任务：
- [x] 编写数据库一致性备份脚本，避免直接拷贝活跃写入中的 SQLite 文件
- [x] 编写正式资源目录、关键配置、日志以及 Nginx 和 systemd 配置的备份脚本
- [x] 补充恢复手册，明确配置、数据库、资源目录、应用启动和巡检的恢复顺序
- [x] 增加备份与恢复日志记录，分别输出执行时间、对象范围和结果摘要
- [x] 执行一次恢复演练，验证公开页面、公开 API、后台 API 和深度健康检查均可恢复访问
- [x] 验证恢复后资源巡检结果可解释，异常资源已被自动下线或从公开查询排除
...
子任务：
- [x] 创建 `tags` 与 `wallpaper_tags` 的迁移脚本，落地标签定义表、关联表和唯一索引
- [x] 实现后台标签列表、创建更新和内容标签绑定接口，校验 `tag_key` 唯一性与标签状态
- [x] 实现后台标签维护页面和内容标签编辑入口，支持查看现有标签关系并提交变更
- [x] 在公开接口中增加标签筛选参数和公开标签列表输出，只返回可公开使用的启用标签
- [x] 增加标签变更后的内容查询刷新逻辑，保证后台与公开端读取到一致结果
- [x] 验证多标签绑定持久化、后台维护、公开筛选和停用标签隐藏是否满足验收标准
...
子任务：
- [x] 抽象采集来源接口，拆分来源标识、拉取入口和字段映射边界，保持 Bing 主链路不变
- [x] 为新增来源补充配置项和来源开关，明确来源类型、请求参数和重试策略
- [x] 实现至少一个 Bing 之外的新来源采集适配器，并输出统一内部壁纸与任务结构
- [x] 扩展内容与任务写入逻辑，确保 `wallpapers` 和 `collection_tasks` 可按 `source_type` 区分来源
- [x] 增加新来源任务日志与结果摘要记录，保证来源类型和处理结果可观测
- [x] 验证 Bing 采集稳定性不受影响，且新来源采集、去重和任务记录满足验收要求
...
子任务：
- [x] 扩展 `image_resources.resource_type` 使用规范，明确原图、缩略图、预览图和下载图的资源类型取值
- [x] 实现原图生成缩略图和预览图的派生处理流程，并写入对应资源记录
- [x] 增加列表页、详情页和下载场景的资源选择逻辑，按页面用途返回合适资源地址
- [x] 实现派生资源失败日志与状态记录，保证后台或日志中可定位失败原因
- [x] 调整公开 API 输出字段，使列表默认返回缩略图、详情区分预览图和下载图
- [x] 验证资源表类型区分、前端资源使用策略和派生失败可观测性是否符合验收标准
...
子任务：
- [x] 抽象统一资源定位接口，隔离本地文件路径和对象存储访问地址的生成方式
- [x] 扩展 `image_resources.storage_backend` 与相关配置，支持本地与 OSS 两类后端标识
- [x] 实现本地资源与 OSS 资源并存时的读取和下载地址生成逻辑，不暴露服务器磁盘路径
- [x] 补充对象存储配置、迁移期目录约定和备份范围说明，保持与现有部署约束一致
- [x] 增加存储切换期间的回退与兼容校验，确保历史本地资源仍可访问
- [x] 验证本地与 OSS 资源同时访问、公开页面资源切换和下载地址隐藏磁盘路径是否符合验收标准
...
子任务：
- [x] 创建 `download_events` 迁移脚本，落地下载登记表及按内容、资源、时间查询的索引
- [x] 实现 `POST /api/public/download-events` 接口，记录下载入口、请求追踪 ID 和跳转地址
- [x] 增加客户端 IP 脱敏摘要、用户代理记录和登记失败降级日志，避免阻塞静态资源主链路
- [x] 实现后台下载统计查询或趋势聚合接口，输出基础下载结果和时间趋势数据
- [x] 创建后台下载统计页面或视图，展示下载量、热门内容和趋势摘要
- [x] 验证下载登记可追踪、统计结果可观测，且文件传输仍由静态资源链路承担主流量
...
子任务：
- [x] 梳理可搜索字段，明确标题、简述、版权说明、描述和标签等关键词来源
- [x] 实现公开搜索参数和后台搜索参数解析，保持两端接口的查询结构清晰可控
- [x] 为公开端实现关键词搜索与筛选组合查询，并继续应用公开状态过滤规则
- [x] 为后台实现关键词与状态联合检索，返回可解释的后台内容结果
- [x] 记录代表性搜索样本与响应时间，检查常规请求是否满足 `1` 秒内返回目标
- [x] 验证相同关键词在公开端和后台端的结果差异可由状态规则解释，且搜索结果稳定可复现
