...
子任务：
- [ ] 定义公开 API 的请求与响应 schema，统一列表、详情、筛选和站点信息接口的返回结构
- [ ] 实现公开列表查询服务，严格应用 `content_status`、`is_public`、资源状态和发布时间窗口过滤规则
- [ ] 实现公开详情与筛选项接口，返回标题、版权、尺寸、地区、下载能力和可选筛选数据
- [ ] 增加分页参数、排序参数和非法参数校验，返回约定的 HTTP 状态码与业务错误码
- [ ] 补充请求 `trace_id` 注入和访问日志记录，保证接口日志可关联
- [ ] 验证列表分页、详情返回、不可下载内容处理和非法参数错误响应是否符合接口约定
