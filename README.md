...
> **本仓库已于 2026-04-14 归档，不再继续维护。**
>
> 项目已完成阶段一至阶段三全部规划能力（T1.1 – T3.6），当前以只读形式保留。如有需要，可自行 fork 后继续开发。
...
- 项目阶段：阶段三能力已完成，`T3.1` 至 `T3.6` 已完成；真实目标机 `139.224.235.228:8000` 已完成 `H5` 长驻部署与 `H4` 首轮 `cron` 闭环验证，`M4` 所需最小告警方案已收敛为“Webhook + 外层巡检/监控”并通过 Server 酱完成真实测试通知，`M5` 所需运维执行记录模板已补充到 [docs/operations-record-templates.md](/home/ops/Projects/BingWall/docs/operations-record-templates.md)，`L3` 所需密码哈希升级路径已补充到 [docs/password-hash-migration.md](/home/ops/Projects/BingWall/docs/password-hash-migration.md)，当前剩余运维缺口集中在日志轮转收口
...
- 当前文档状态：系统设计、模块说明、数据模型、API 约定、部署运行说明、前端构建与静态资源边界、密码哈希升级迁移设计、运维执行记录模板、搜索与查询性能基线报告、项目状态、整改清单与 `H4` 目标机执行记录已同步到当前实现
...
- [前端构建与静态资源边界](docs/frontend-build-boundary.md)
- [密码哈希升级迁移设计](docs/password-hash-migration.md)
- [运维执行记录模板](docs/operations-record-templates.md)
- [搜索与查询性能基线报告](docs/benchmark-report.md)
...
- `web/src` 只存放 Tailwind 输入文件和 `@source` 声明，是样式源码目录
- `web/public/assets/site.js`、`web/admin/assets/admin.js`、`web/admin/assets/modules/*.js`、`web/admin/assets/pages/*.js` 是手工维护的前端源码
- `web/public/assets/site.css` 与 `web/admin/assets/admin.css` 是构建产物，由 Tailwind CLI 从 `web/src` 生成
- 若你修改了 `web/src/*`，或在 `app/web/routes.py`、`web/public/assets/site.js`、`web/admin/assets/*.js` 里新增/替换了 Tailwind 类名，就必须重新构建 CSS
- 当前部署、`make verify-deploy` 和备用 `nginx` 方案都会直接读取仓库中的静态文件，不会在运行前自动执行前端构建；因此样式变更必须连同构建产物一起提交
- 详细约定见 [docs/frontend-build-boundary.md](/home/ops/Projects/BingWall/docs/frontend-build-boundary.md)
