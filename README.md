...
当前仓库已提供最小后端启动与验证命令。本地开发环境请先确保已安装 `uv`：
...
- `uv python install 3.14`、`uv venv --python 3.14 .venv`、`uv pip install --python .venv/bin/python -e ".[dev]"` 开发环境准备入口，以及 `make db-migrate`、`make verify`、`make run` 统一命令入口
...
- `/` 首页、`/wallpapers` 列表页、`/wallpapers/{id}` 详情页三个公开页面；其中首页已直接展示“今日壁纸 API”和“随机壁纸 API”两个快捷入口，`/wallpapers` 还新增 `zh-CN / en-US / ja-JP` 固定市场下拉，以及单独的日期选择器；前者调用 `/api/public/wallpapers/by-market/{market_code}` 查看对应市场最新公开壁纸，后者调用 `/api/public/wallpapers/by-date/{wallpaper_date}` 精确查找单张公开壁纸
...
- `/wallpapers` 与 `/admin/wallpapers` 页面已新增关键词输入框，并继续只通过现有公开 / 后台 API 取数；其中公开列表页额外补充 `zh-CN / en-US / ja-JP` 固定市场下拉，单独调用 `/api/public/wallpapers/by-market/{market_code}` 展示所选市场最新一张公开壁纸，同时新增日期选择器，单独调用 `/api/public/wallpapers/by-date/{wallpaper_date}` 精确查找当天公开壁纸
...
- 访问 `/wallpapers` 后，页面顶部会先显示“按市场查看最新壁纸”区域，固定提供 `zh-CN`、`en-US`、`ja-JP` 三个选项。
- 切换该下拉框时，前端会调用 `/api/public/wallpapers/by-market/{market_code}`，只刷新该市场结果卡片，不改变下方分页列表的筛选语义。
...
- 目标机执行 `make install-cron` 后的首轮运行确认
...
sudo -u bingwall bash -lc 'cd /opt/bingwall/app && uv venv --python 3.14 .venv'
sudo -u bingwall bash -lc 'cd /opt/bingwall/app && uv pip install --python .venv/bin/python -e .'
