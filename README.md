...
- 项目阶段：阶段三能力已完成，`T3.1` 至 `T3.6` 已完成；真实目标机 `139.224.235.228:8000` 已完成 `H5` 长驻部署与 `H4` 首轮 `cron` 闭环验证，当前剩余运维缺口集中在告警方案、执行记录模板与日志轮转收口
...
- 当前文档状态：系统设计、模块说明、数据模型、API 约定、部署运行说明、项目状态、整改清单与 `H4` 目标机执行记录已同步到当前实现
...
- 仓库新增独立的 `Verify Deploy` 工作流；当 `deploy/**`、`scripts/verify_t1_6.py`、`app/**`、`web/public/assets/**` 等部署验收相关文件变化时，会在带 `bingwall-deploy` 标签的 Linux self-hosted runner 上执行 `make verify-deploy`，也支持手工 `workflow_dispatch`
...
- `Verify Deploy` 依赖带 `bingwall-deploy` 标签的 Linux self-hosted runner，且该 runner 账号必须能访问 `docker`，并具备可用的 `systemd --user` 会话；仓库统一入口脚本为 `bash scripts/github/run_verify_deploy.sh`
- 远端入口脚本默认会把部署验收临时监听端口切到 `127.0.0.1:28000` 和 `28080`，避免与机器上长期运行的 `bingwall-api.service` 或其他本地服务冲突；如需改写，可设置 `BINGWALL_VERIFY_DEPLOY_APP_PORT` 与 `BINGWALL_VERIFY_DEPLOY_NGINX_PORT`
