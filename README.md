...
- 项目阶段：阶段二进行中，`T2.3` 已完成，下一优先级为 `T2.4`
- 当前代码状态：已完成最小后端工程骨架、统一配置入口、最小 FastAPI 应用、SQLite 迁移基线、数据库初始化命令、Bing 采集与资源入库主链路、公开 API 最小集、基础公开前端、`T1.6` 自动化部署验收，以及 `T2.1` 管理员认证与会话控制、`T2.2` 后台内容管理 API / 页面与审计查询、`T2.3` 手动采集任务与后台观测闭环
...
- `/api/health/ready` 与 `/api/health/deep`
- 备份恢复脚本
- 目标机 cron 安装与计划配置
