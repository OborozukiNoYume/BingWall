...
- `/` 首页、`/wallpapers` 列表页、`/wallpapers/{id}` 详情页三个公开页面；其中首页已直接展示“今日壁纸 API”和“随机壁纸 API”两个快捷入口，`/wallpapers` 还新增 `zh-CN / en-US / ja-JP` 固定市场下拉，可直接调用 `/api/public/wallpapers/by-market/{market_code}` 查看对应市场最新公开壁纸
...
- `/wallpapers` 与 `/admin/wallpapers` 页面已新增关键词输入框，并继续只通过现有公开 / 后台 API 取数；其中公开列表页额外补充 `zh-CN / en-US / ja-JP` 固定市场下拉，单独调用 `/api/public/wallpapers/by-market/{market_code}` 展示所选市场最新一张公开壁纸
